...
19. **Real-Time Dynamic Route Shortening & Calibrated Approach Speed (0.1 km / 2.5s)**:
    - Implemented `RouteRepository.getRemainingRoutePoints` dynamically slicing the OpenStreetMap road polyline in real time as the driver advances from Woodmead toward the rider. The traversed dotted road behind the vehicle is dynamically removed, while driver speed is calibrated to exactly 0.1 km per 2.5 seconds with clean live distance and ETA telemetry.
